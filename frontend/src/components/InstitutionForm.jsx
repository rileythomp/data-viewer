...
export default function InstitutionForm({ onSubmit, onCancel, initialData = null, accounts = [] }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isCalculated && formulaItems.length === 0) {
      setError('Calculated institution must have at least one account in the formula');
      return;
    }
    ...
    try {
      await onSubmit(name.trim(), description.trim(), color, isCalculated, formulaData);
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="group-form">
      <h3>{initialData ? 'Edit Institution' : 'Create New Institution'}</h3>
      ...
      <div className="form-group">
        <label htmlFor="institutionDescription">Description</label>
        <textarea
          id="institutionDescription"
          value={description}
          onChange={(e) => setDescription(e.target.value)}
          placeholder="Optional description for this institution"
          rows={2}
        />
      </div>
      ...
      <div className="form-actions">
        ...
        <button type="submit" className="btn-primary" disabled={isSubmitting}>
          {isSubmitting ? 'Saving...' : initialData ? 'Save Changes' : 'Create Institution'}
        </button>
      </div>
    </form>
  );
